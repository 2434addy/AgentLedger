...
export class EventsService {
  ...
  async bulkCreate(orgId: string, dto: CreateEventsDto) {
    const saved = await this.dataSource.transaction(async (manager) => {
      ...
      // Validate all agentIds belong to this org
      const agentIdsToValidate = [...new Set(dto.events.map(e => e.agentId))];
      if (agentIdsToValidate.length > 0) {
        const validAgents = await agentRepo
          .createQueryBuilder('a')
          .select('a.id')
          .where('a.id IN (:...ids)', { ids: agentIdsToValidate })
          .andWhere('a.orgId = :orgId', { orgId })
          .getMany();
        if (validAgents.length !== agentIdsToValidate.length) {
          throw new BadRequestException('One or more agentIds do not belong to your organisation');
        }
      }
  ...
  /**
   * Convert orgId (UUID) to a signed bigint for PostgreSQL advisory lock.
   * XOR-folds the full 128-bit UUID into 63 bits (signed bigint range).
   */
  private orgIdToLockKey(orgId: string): string {
    const hex = orgId.replace(/-/g, '');
    const hi = BigInt('0x' + hex.slice(0, 16));
    const lo = BigInt('0x' + hex.slice(16, 32));
    const folded = (hi ^ lo) & BigInt('0x7FFFFFFFFFFFFFFF');
